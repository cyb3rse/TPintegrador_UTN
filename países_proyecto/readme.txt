...
Su objetivo es permitir que el usuario pueda buscar información de diferentes países a partir de un archivo CSV que contiene datos como el nombre, la población, la superficie y el continente al que pertenecen. 
La aplicación muestra los resultados de forma ordenada en una página web simple y clara.
Para poder usar el proyecto, primero es necesario tener instalado Python 3.10 o superior, ya que el código está hecho en esa versión o una más reciente. También se recomienda tener Git, aunque no es obligatorio, solo en caso de que se quiera clonar el repositorio directamente desde GitHub.
